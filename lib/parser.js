...
"use strict";
var util=__webpack_require__(/*! ./util */ "./src/util.js");var buildOptions=__webpack_require__(/*! ./util */ "./src/util.js").buildOptions;var xmlNode=__webpack_require__(/*! ./xmlNode */ "./src/xmlNode.js");var TagType={OPENING:1,CLOSING:2,SELF:3,CDATA:4};var attrstr_regex='((\\s*[\\w\\-._:]+(=((\'([^\']*)\')|("([^"]*)")))?)*)\\s*';var regx='<((!\\[CDATA\\[([\\s\\S]*?)(]]>))|(([\\w:\\-._]*:)?([\\w:\\-._]+))'+attrstr_regex+'(\\/)?>|((\\/)(([\\w:\\-._]*:)?([\\w:\\-._]+))\\s*>))([^<]*)';//'<((!\\[CDATA\\[([\\s\\S]*?)(]]>))|(([\\w:\\-._]*:)?([\\w:\\-._]+))([^>]*)>|((\\/)(([\\w:\\-._]*:)?([\\w:\\-._]+))\\s*>))([^<]*)';
...
var xmlObj=new xmlNode('!xml');var currentNode=xmlObj;regx=regx.replace(/\[\\w/g,'['+options.localeRange+'\\w');var tagsRegx=new RegExp(regx,'g');var tag=tagsRegx.exec(xmlData);var nextTag=tagsRegx.exec(xmlData);while(tag){//console.log(tag)
var tagType=checkForTagType(tag);//console.log(tagType)
if(tagType===TagType.CLOSING){//add parsed data to parent node
if(currentNode.parent&&tag[22]){currentNode.parent.val=util.getValue(currentNode.parent.val)+''+processTagValue(tag[22],options);}if(options.stopNodes.length&&options.stopNodes.includes(currentNode.tagname)){currentNode.child=[];if(currentNode.attrsMap==undefined){currentNode.attrsMap={};}currentNode.val=xmlData.substr(currentNode.startIndex+1,tag.index-currentNode.startIndex-1);}currentNode=currentNode.parent;}else if(tagType===TagType.CDATA){if(options.cdataTagName){//add cdata node
...
if(tag[22]){currentNode.val+=processTagValue(tag[22],options);}}else{currentNode.val=(currentNode.val||'')+(tag[3]||'')+processTagValue(tag[22],options);}}else if(tagType===TagType.SELF){if(currentNode&&tag[22]){currentNode.val=util.getValue(currentNode.val)+''+processTagValue(tag[22],options);}var _childNode=new xmlNode(options.ignoreNameSpace?tag[7]:tag[5],currentNode,'');/* if (tag[8] && tag[8].length > 0) {
        tag[8] = tag[8].substr(0, tag[8].length - 1);
      } */_childNode.attrsMap=buildAttributesMap(tag[8],options);currentNode.addChild(_childNode);}else{//TagType.OPENING
var _childNode2=new xmlNode(options.ignoreNameSpace?tag[7]:tag[5],currentNode,processTagValue(tag[22],options));if(options.stopNodes.length&&options.stopNodes.includes(_childNode2.tagname)){_childNode2.startIndex=tag.index+tag[1].length;}_childNode2.attrsMap=buildAttributesMap(tag[8],options);currentNode.addChild(_childNode2);currentNode=_childNode2;}tag=nextTag;nextTag=tagsRegx.exec(xmlData);}return xmlObj;};function processTagValue(val,options){if(val){if(options.trimValues){val=val.trim();}val=options.tagValueProcessor(val);val=parseValue(val,options.parseNodeValue,options.parseTrueNumberOnly);}return val;}function checkForTagType(match){if(match[4]===']]>'){return TagType.CDATA;}else if(match[18]==='/'){return TagType.CLOSING;}else if(match[16]==='/'){return TagType.SELF;}else{return TagType.OPENING;}}function resolveNameSpace(tagname,options){if(options.ignoreNameSpace){var tags=tagname.split(':');var prefix=tagname.charAt(0)==='/'?'/':'';if(tags[0]==='xmlns'){return'';}if(tags.length===2){tagname=prefix+tags[1];}}return tagname;}function parseValue(val,shouldParse,parseTrueNumberOnly){if(shouldParse&&typeof val==='string'){var parsed;if(val.trim()===''||isNaN(val)){parsed=val==='true'?true:val==='false'?false:val;}else{if(val.indexOf('0x')!==-1){//support hexa decimal
