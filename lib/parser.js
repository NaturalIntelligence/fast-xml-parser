...
var closeIndex=xmlData.indexOf(">",i);var tagName=xmlData.substring(i+2,closeIndex).trim();if(options.ignoreNameSpace){var colonIndex=tagName.indexOf(":");if(colonIndex!==-1){tagName=tagName.substr(colonIndex+1);}}/* if (currentNode.parent) {
        ...
        } */if(currentNode){if(currentNode.val){currentNode.val=util.getValue(currentNode.val)+''+processTagValue(tagName,textData,options);}else{currentNode.val=processTagValue(tagName,textData,options);}}if(options.stopNodes.length&&options.stopNodes.includes(currentNode.tagname)){currentNode.child=[];if(currentNode.attrsMap==undefined){currentNode.attrsMap={};}currentNode.val=xmlData.substr(currentNode.startIndex+1,i-currentNode.startIndex-1);}currentNode=currentNode.parent;textData="";i=closeIndex;}else if(xmlData[i+1]==='?'){i=xmlData.indexOf("?>",i)+1;}else if(xmlData[i+1]==='!'&&xmlData[i+2]==='-'){i=xmlData.indexOf("-->",i)+2;}else if(xmlData[i+1]==='!'&&xmlData[i+2]==='D'){var _closeIndex=xmlData.indexOf(">",i);var tagExp=xmlData.substr(i,_closeIndex);if(tagExp.indexOf("[")){i=xmlData.indexOf("]>",i)+1;}else{i=_closeIndex;}}else if(xmlData[i+2]==='['){var _closeIndex2=xmlData.indexOf("]]>",i);var _tagExp=xmlData.substring(i+9,_closeIndex2);//considerations
