...
var _closeIndex3=closingIndexForOpeningTag(xmlData,i);//const closeIndex = xmlData.indexOf(">",i);
var _tagExp2=xmlData.substring(i+1,_closeIndex3);var separatorIndex=_tagExp2.indexOf(" ");var _tagName=_tagExp2;if(separatorIndex!==-1){_tagName=_tagExp2.substr(0,separatorIndex);_tagExp2=_tagExp2.substr(separatorIndex+1);}if(options.ignoreNameSpace){var _colonIndex=_tagName.indexOf(":");if(_colonIndex!==-1){_tagName=_tagName.substr(_colonIndex+1);}}//save text to parent node
if(currentNode&&textData){if(currentNode.tagname!=='!xml'){currentNode.val=util.getValue(currentNode.val)+''+processTagValue(currentNode.tagname,textData,options);}}if(_tagExp2.lastIndexOf("/")===_tagExp2.length-1){//selfClosing tag
...
var _childNode2=new xmlNode(_tagName,currentNode);if(options.stopNodes.length&&options.stopNodes.includes(_childNode2.tagname)){_childNode2.startIndex=_closeIndex3;}if(_tagName!==_tagExp2){_childNode2.attrsMap=buildAttributesMap(_tagExp2,options);}currentNode.addChild(_childNode2);currentNode=_childNode2;}textData="";i=_closeIndex3;}}else{textData+=xmlData[i];}}return xmlObj;};function closingIndexForOpeningTag(data,i){var attrBoundary;for(var index=i;index<data.length;index++){var ch=data[index];if(attrBoundary){if(ch===attrBoundary)attrBoundary="";//reset
}else if(ch==='"'||ch==="'"){attrBoundary=ch;}else if(ch==='>'){return index;}}}exports.getTraversalObj=getTraversalObj;
