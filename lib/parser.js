...
var localRangeRegex=new RegExp("["+options.localeRange+"]");if(localRangeRegex.test("<#$'\"\\\/:0")){return getErrorObject('InvalidOptions','Invalid localeRange',1);}var tags=[];var tagFound=false;if(xmlData[0]==="\uFEFF"){// check for byte order mark (BOM)
xmlData=xmlData.substr(1);}var regxAttrName=new RegExp("^["+options.localeRange+"_]["+options.localeRange+"0-9\\-\\.:]*$");var regxTagName=new RegExp("^(["+options.localeRange+"_])["+options.localeRange+"0-9\\.\\-_:]*$");for(var i=0;i<xmlData.length;i++){if(xmlData[i]==='<'){//starting of tag
...
return getErrorObject('InvalidAttr',"boolean attribute '"+matches[i][2]+"' is not allowed.",getPositionFromMatch(attrStr,matches[i][0]));}/* else if(matches[i][6] === undefined){//attribute without value: ab=
                ...
                } */var attrName=matches[i][2];if(!validateAttrName(attrName,regxAttrName)){return getErrorObject('InvalidAttr',"Attribute '"+attrName+"' is an invalid name.",getPositionFromMatch(attrStr,matches[i][0]));}if(!attrNames.hasOwnProperty(attrName)){//check for duplicate attribute.
attrNames[attrName]=1;}else{return getErrorObject('InvalidAttr',"Attribute '"+attrName+"' is repeated.",getPositionFromMatch(attrStr,matches[i][0]));}}return true;}function getErrorObject(code,message,lineNumber){return{err:{code:code,msg:message,line:lineNumber}};}function validateAttrName(attrName,regxAttrName){// const validAttrRegxp = new RegExp(regxAttrName);
...
return!util.doesNotMatch(tagname,regxTagName);}//this function returns the line number for the character at the given index
...
cdataTagName:false,cdataPositionChar:'\\c',localeRange:'',tagValueProcessor:function tagValueProcessor(a,tagName){return a;},attrValueProcessor:function attrValueProcessor(a,attrName){return a;},stopNodes:[]//decodeStrict: false,
